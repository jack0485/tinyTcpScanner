...
/* 命令行参数：-m 最大端口, -a 目标主机IP地址, -n 最大线程数 */
int main(int argc, char **argv) {
    ...
    // 检查参数个数
    if (argc != 7) {
        printf("Usage: [-m] [max_port] [-a] [serv_address] [-n] [pthread_number]\n");
        exit(1);
    }
    ...
    // 如果不能整除，说明线程少了，需要增加一个线程用于处理余下的端口
    ...
    // 创建线程，根据最大端口号和线程数分配每个线程的端口扫描区间
    for (i = 0; i < thread_num; i++) {
        port_segment portinfo;
        portinfo.dest_ip = dest_ip;
        portinfo.min_port = i * seg_len + 1;    // 端口区间的起始位置
        if (i == thread_num - 1) {
            portinfo.max_port = max_port;
        } else {
            portinfo.max_port = portinfo.min_port + seg_len - 1;    // 端口区间的结束位置
        }
        // 创建线程
